...
# Plots frequency of MTMs, gapes, and no movement as determined by 
    # Blech EMG Classifier averaged across trials 
    # Saves behavior frequency artifacts for aggregate version of script
...
filter_licl_conc = '0.6M_LiCl'  # set to '0.15M', '0.6M', or None for all
    ...
    if len(days) > 0:
        ...
        for i_row, animal in enumerate(animals):
            ...
            for i_col, day in enumerate(days):
                ...
                exp_day_type = session_group['exp_day_type'].iloc[0]
                ...
                for clust in clusters_to_plot:
                    ...
                    x_list.append(time_bins)
                    y_list.append(freq_trace_smoothed)
        
                #save atrifacts for each animal for aggregate plots in future
                ...
                #Save artifacts for aggregate plotting in extract_emg_from_transition_aggregate.py
                artifact_dir = '/media/cmazzio/large_data/behavior_frequency_classifier_artifacts'
                artifact_save_path = os.path.join(artifact_dir, f'{basename}_{taste_name}_behavior_freq_classifier_agg_data.pkl')
                with open(artifact_save_path, 'wb') as f:
                      pickle.dump(fin_output, f)   
                print(f"saved artifact: {taste_name}, {basename}")
                # -----------------------------
...
for taste_name, group in df_to_plot.groupby('taste_name'):
    ...
    if len(days) > 0:
        ...
        for i_row, animal in enumerate(animals):
            ...
            if animal not in all_animal_artifacts:
                all_animal_artifacts[animal] = {
                    "animal": animal,
                    "tastes": {}
                }

            animal_artifact = all_animal_artifacts[animal]
            first_visible_ax = None
            ...
            for i_col, day in enumerate(days):
                ...
                # --- ensure taste exists ---
                if taste_name not in animal_artifact["tastes"]:
                    animal_artifact["tastes"][taste_name] = {"basenames": {}}

                # --- ensure basename exists ---
                if basename not in animal_artifact["tastes"][taste_name]["basenames"]:
                    animal_artifact["tastes"][taste_name]["basenames"][basename] = {
                ...
                for clust in clusters_to_plot:
                    ...
                    # --- STORE WITHOUT extra day level ---
                    animal_artifact["tastes"][taste_name]["basenames"][basename]["movement_type"][clust] = {
                        "x": time_bins.copy(),
                        "y": freq_trace_smoothed.copy()
                    }

# Save artifacts
artifact_dir = '/media/cmazzio/large_data/behavior_frequency_classifier_artifacts'
for animal, artifact in all_animal_artifacts.items():
    artifact_path = os.path.join(artifact_dir, f'{animal}_behavior_freq_classifier.pkl')
